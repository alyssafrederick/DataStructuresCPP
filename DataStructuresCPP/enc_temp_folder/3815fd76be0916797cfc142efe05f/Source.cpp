...
int main()
{
	...
	std::cout << trie.Remove("hey") << std::endl;					//1 = true
	std::cout << trie.SearchNode("hey")->Letter << std::endl;		//y (bc that prefix is still there even though its not a word)
	std::cout << trie.Contains("hello") << std::endl;				//1 = true
	trie.Insert("he");
	auto matches1 = trie.GetAllMatchingPrefix("he");
	trie.Insert("hi");
	trie.Insert("howdy");
